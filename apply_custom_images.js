...
const file = 'src/data/templeMetadata.ts';
...
for (const { term, url } of replacements) {
  // Regex to match the exact slug and replace the image url
  // e.g. "dwarkadhish": { ... image: "..." }
  const regex = new RegExp(`("${term}"\\s*:\\s*{[^}]*?image\\s*:\\s*)"([^"]*)"`, 'g');
  let matched = false;
  ...
  if (!matched) {
    console.log(`Failed to match slug: ${term}`);
  }
}
